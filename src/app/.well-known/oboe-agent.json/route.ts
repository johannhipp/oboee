export async function GET(request: Request) {
  ...
  const fundingTokenAddress = process.env.MPP_FUNDING_TOKEN_ADDRESS?.trim() || "configured-at-runtime";
  return Response.json({
    ...
    payments: { protocol: "MPP", network: process.env.NEXT_PUBLIC_MPP_NETWORK ?? "configured-at-runtime", token: fundingTokenAddress, amounts: "unsigned base-unit decimal strings", receiptAuthority: "server-verified only" },
