...
export function AdjudicationControls({ disputeId, state, criteria }: { disputeId: string; state: string; criteria: Criterion[] }) {
  ...
  if (state === "open") return <div><button className="border-b border-foreground pb-1 font-mono text-xs" disabled={busy} onClick={async () => { setBusy(true); try { await send(`/api/v2/adjudications/${disputeId}/claim`); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Claim failed."); } finally { setBusy(false); } }}>claim with recent passkey</button>{message ? <p role="status" className="mt-2 border-y border-border py-3 text-sm">{message}</p> : null}</div>;
  return <form className="mt-4 space-y-4" onSubmit={async (event) => { event.preventDefault(); setBusy(true); const fields = new FormData(event.currentTarget); try { await send(`/api/v2/adjudications/${disputeId}/resolve`, { resolution: fields.get("resolution"), criterionResults: criteria.map((criterion) => ({ criterionId: criterion.criterionId, result: results[criterion.criterionId] })), rationale: fields.get("rationale"), publicRedaction: fields.get("publicRedaction"), conflictDeclared: false }); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Resolution failed."); } finally { setBusy(false); } }}>
    <div className="divide-y divide-border border-y border-border">{criteria.map((criterion) => <label key={criterion.criterionId} className="grid gap-2 py-3 sm:grid-cols-[1fr_auto]"><span><span className="block text-sm font-medium">{criterion.title}</span><span className="block text-xs text-muted-foreground">{criterion.weightBps / 100}% · {criterion.passCondition}</span></span><select value={results[criterion.criterionId]} onChange={(event) => setResults((current) => ({ ...current, [criterion.criterionId]: event.target.value as typeof results[string] }))} className="border-0 border-b border-border bg-transparent px-0 py-2 text-sm outline-none focus:border-foreground"><option value="not_run">not run</option><option value="passed">passed</option><option value="failed">failed</option></select></label>)}</div>
    <label className="block text-sm">Fixed resolution<select required name="resolution" className="mt-1 w-full border-0 border-b border-border bg-transparent px-0 py-2 outline-none focus:border-foreground"><option value="insufficient_evidence">insufficient evidence</option><option value="clear_hold">clear hold</option><option value="request_revision">request revision</option><option value="accept_partial">accept partial</option><option value="block_harmful">block harmful</option><option value="reject_fraud">reject fraud</option><option value="confirm_abandonment">confirm abandonment</option></select></label>
    <label className="block text-sm">Private adjudicator rationale<textarea required name="rationale" rows={3} className="mt-1 w-full resize-y border-0 border-b border-border bg-transparent px-0 py-2 outline-none focus:border-foreground" /></label>
    <label className="block text-sm">Public redaction<textarea required name="publicRedaction" rows={2} className="mt-1 w-full resize-y border-0 border-b border-border bg-transparent px-0 py-2 outline-none focus:border-foreground" /></label>
    ...
    <button disabled={busy} className="w-fit border-b border-foreground pb-1 font-mono text-xs">resolve with recent passkey</button>{message ? <p role="status" className="border-y border-border py-3 text-sm">{message}</p> : null}
  </form>;
}
