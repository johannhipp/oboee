...
export function FixtureUploader({ onRegistered }: { onRegistered: (fixtureVersionId: string) => void }) {
  ...
  }}><div className="sm:col-span-2"><h2 className="font-medium">Fixture registration</h2><p className="text-xs leading-5 text-muted-foreground">Register an inert bundle with immutable hashes. Execution remains isolated from the request workflow.</p></div><label className="text-sm">Bundle<input required type="file" name="bundle" accept=".zip,application/zip,application/json" className="mt-1 block w-full text-sm" /></label><label className="text-sm">Visibility<select name="visibility" className="mt-1 h-10 w-full border-0 border-b border-border bg-transparent px-0 text-sm outline-none focus:border-foreground"><option value="restricted">restricted</option><option value="public">public</option></select></label><label className="text-sm sm:col-span-2">Manifest JSON<textarea required name="manifest" rows={3} className="mt-1 w-full border-0 border-b border-border bg-transparent px-0 py-2 font-mono text-xs outline-none focus:border-foreground" /></label><label className="text-sm sm:col-span-2">Environment contract<input required name="environmentContract" placeholder="node-22, no network, disposable workspace" className="mt-1 w-full border-0 border-b border-border bg-transparent px-0 py-2 text-sm outline-none focus:border-foreground" /></label><button disabled={busy} className="w-fit border-b border-foreground pb-1 font-mono text-sm disabled:opacity-50">{busy ? "Registering…" : "Register fixture"}</button>{message ? <p role="status" className="self-center text-sm break-all">{message}</p> : null}</form>;
}
