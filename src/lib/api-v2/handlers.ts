...
export const getMyWork = v2Authorized("rfs:read", async ({ requestId, authentication }) => v2Success({ requestId, data: await fetchPrincipalQuery(anyApi.workspace.myWork, {}, authentication), capabilities: [], links: { activity: "/api/v2/me/activity" } }));
export const getMyActivity = v2Authorized("rfs:read", async ({ request, requestId, authentication }) => { const url = new URL(request.url); const result = await fetchPrincipalQuery(anyApi.workspace.myActivity, { afterSequence: Number(url.searchParams.get("cursor") ?? 0), limit: Number(url.searchParams.get("limit") ?? 50) }, authentication); return v2Success({ requestId, data: result.items, capabilities: [], links: { self: request.url }, page: { nextCursor: result.nextCursor } }); });
export const getMyObligations = v2Authorized("settlement:read", async ({ requestId, authentication }) => { const rows = await fetchPrincipalQuery(anyApi.settlements.myObligations, {}, authentication); return v2Success({ requestId, data: rows.map((item: { obligation: Record<string, unknown>; transfer: unknown }) => ({ ...item, capabilities: obligationCapabilities(String(item.obligation.state)) })), capabilities: [], links: { earnings: "/api/v2/me/earnings" } }); });
export const getMyEarnings = v2Authorized("settlement:read", async ({ requestId, authentication }) => v2Success({ requestId, data: await fetchPrincipalQuery(anyApi.workspace.myEarnings, {}, authentication), capabilities: [], links: {} }));
